...
We utilize a Gradient Boosting Machine model, chosen for its robustness in handling various types of data and its ability to model complex interactions and nonlinear relationships between predictors and the response variable.
...
Data for this study is sourced from Open Data Toronto: <https://open.toronto.ca/dataset/shootings-firearm-discharges/>, detailing all recorded shootings and firearm discharges within the city limits from 2004 to 2024.

## Data Cleaning

Data cleaning was performed using R[@citeR] packages such as `tidyverse` and `lubridate`. The `occ_date` variable was converted to a numeric format to fit the model requirements, and categorical variables were transformed using appropriate factor conversions.
...
The analysis employed a Gradient Boosting Machine (GBM) model using the gbm package in R, designed to predict the weighted scores of shootings and firearm discharges in Toronto based on multiple time-related and geographical predictors. The weighted score is a computed metric that assigns double weight to deaths compared to injuries, highlighting the gravity of incidents resulting in fatalities.
...
The model highlighted significant predictors and their relative influence on the severity of shootings. `Division` emerged as a particularly influential predictor, indicating that certain areas within Toronto experience distinctly different levels of shooting severity.
...
Based on the model's findings, policy recommendations are proposed to enhance public safety and optimize police resource allocation.

# Conclusion

This study provides a comprehensive analysis of the factors affecting the severity of shootings in Toronto, offering actionable insights for policymakers and law enforcement agencies.
